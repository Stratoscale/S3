            ...
            return async.forEachOfSeries(dataLocator,
                ...
                function copyPart(part, index, cb) {
                    return data.get(part, log, (err, stream) => {
                        ...
                        // objectLocationConstraint is location of MPU part
                        // will be copied to
                        if (externalBackends[config.locationConstraints
                        [objectLocationConstraint].type]) {
                            return multipleBackendGateway.uploadPart(null, null,
                            hashedStream, numberPartSize,
                            objectLocationConstraint, destObjectKey, uploadId,
                            partNumber, destBucketName, log, err => {
                                if (err) {
                    ...
                    if (err) {
                        if (err === skipError) {
                            return next(skipError, destBucketMD, totalHash,
