...
function getObject(backend, tagCheck, isEmpty, cb) {
    s3.getObject({ Bucket: bucket, Key: key }, (err, res) => {
        ...
        if (backend === 'aws-test') {
            setTimeout(() => {
                awsS3.getObject({ Bucket: awsBucket, Key: key }, (err, res) => {
                    assert.equal(err, null);
                    if (isEmpty) {
                        assert.strictEqual(res.ETag, `"${emptyMD5}"`);
                    } else {
...
describeSkipIfNotMultiple.only('Object tagging with multiple backends',
function testSuite() {
    this.timeout(50000);
    withV4(sigCfg => {
        beforeEach(() => {
